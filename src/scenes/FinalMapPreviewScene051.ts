...
export class FinalMapPreviewScene051 extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.players = gameSession.players.map((profile, index) => ({
      ...
      nodeId: FINAL_MAP_PREVIEW_051.readyNodeId,
      money: 200,
      laps: 0,
    }));
    ...
    this.cameras.main.setZoom(FINAL_MAP_PREVIEW_051.normalFollowZoom);

    this.showToast('0.1.51 DRAFT D • 3 ngã rẽ + map thoáng + camera gần');
  }
  ...
  private drawNode(node: BoardNode): void {
    ...
    const isReady = node.id === FINAL_MAP_PREVIEW_051.readyNodeId;
  ...
  private createControls(): void {
    this.rollButton = this.add.rectangle(640, 678, 188, 42, 0xef4545, 1)
      .setStrokeStyle(3, 0xffffff, 1).setScrollFactor(0).setDepth(320).setInteractive({ useHandCursor: true });
    ...
    const overview = this.add.text(640, 24, '🗺️ TỔNG QUAN', {
      fontFamily: 'Arial, sans-serif', fontSize: '12px', fontStyle: 'bold', color: '#202020', backgroundColor: '#fffaf0', padding: { x: 10, y: 6 },
    }).setOrigin(0.5).setScrollFactor(0).setDepth(321).setInteractive({ useHandCursor: true });
  ...
  private async handleRoll(): Promise<void> {
    ...
    try {
      ...
      for (let step = 0; step < roll; step += 1) {
        ...
        if (edge.to === FINAL_MAP_PREVIEW_051.readyNodeId) player.laps += 1;
  ...
  private chooseRoute(player: PreviewPlayer, edges: BoardEdge[]): Promise<BoardEdge> {
    ...
    this.cameras.main.zoomTo(FINAL_MAP_PREVIEW_051.branchDecisionZoom, 240, 'Sine.easeInOut');
    this.cameras.main.pan(avgX, avgY, 240, 'Sine.easeInOut');

    return new Promise((resolve) => {
      const root = this.add.container(640, 560).setScrollFactor(0).setDepth(500);
      ...
      edges.forEach((edge, index) => {
        ...
        button.on('pointerdown', () => {
          root.destroy();
          this.cameras.main.zoomTo(FINAL_MAP_PREVIEW_051.normalFollowZoom, 240, 'Sine.easeInOut');
  ...
  private async resolveLanding(player: PreviewPlayer): Promise<void> {
    ...
    if (player.nodeId === FINAL_MAP_PREVIEW_051.jailGateNodeId) {
      player.holding = 'jail';
      player.nodeId = FINAL_MAP_PREVIEW_051.jailNodeId;
      this.showToast('🚔 Dính cổng Nhà tù → vào Đồn cảnh sát');
      await this.moveTokenTo(player, player.nodeId, 430);
      return;
    }
    if (player.nodeId === FINAL_MAP_PREVIEW_051.hospitalGateNodeId) {
      player.holding = 'hospital';
      player.nodeId = FINAL_MAP_PREVIEW_051.hospitalNodeId;
      this.showToast('🏥 Dính cổng Bệnh viện → nhập viện');
      await this.moveTokenTo(player, player.nodeId, 430);
      return;
    }
    if (player.nodeId === FINAL_MAP_PREVIEW_051.lotteryNodeId) {
  ...
  private focusCurrentPlayer(animated: boolean): void {
    ...
    if (!token) return;
    this.cameras.main.zoomTo(FINAL_MAP_PREVIEW_051.normalFollowZoom, animated ? 300 : 0, 'Sine.easeInOut');
  ...
  private showOverview(): void {
    ...
    camera.zoomTo(FINAL_MAP_PREVIEW_051.overviewZoom, 280, 'Sine.easeInOut');
    camera.pan(WORLD_W / 2, WORLD_H / 2, 280, 'Sine.easeInOut');
    this.time.delayedCall(2200, () => this.focusCurrentPlayer(true));
  }
