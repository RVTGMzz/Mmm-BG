...
export class LocalLobbyScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(228, 73, 'FIRST PLAYTEST • MVP 0.1.42', {
    ...
    this.add.text(228, 109, '1 vòng/người • Event Audio • Final Podium • Face Reactions.', {
      fontFamily: 'Arial, sans-serif',
      fontSize: '15px',
      color: '#6d655b',
    });
    ...
    root.innerHTML = `
      ...
      <div style="margin-top:12px;padding:10px 14px;border:2px solid #202020;border-radius:14px;background:#fff4d6;font-size:12px;line-height:1.45;font-weight:700;">
        👑 0.1.42: podium ưu tiên mặt vui cho hạng 1, mặt quạu cho hạng cuối và thêm crown/spark spotlight cố định cho mọi người đồng hạng nhất.
      </div>
      <p id="lobby-status" class="lobby-status">${broadcastReady
        ? '✅ Roll For Order + Job Dice + Salary + Mini Games + Event Audio + Final Podium + Face Reactions + 2-tab local sẵn sàng.'
