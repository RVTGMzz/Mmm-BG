...
import { createEmptyHostAuthority, type ClientIntentType } from '../core/authority';
import { getBoardNode, getOutgoingEdges, validateBoardDefinition } from '../core/board';
import { browserSession } from '../core/browserSession';
import { type CardDefinition, getValidTargets } from '../core/cards';
import { computeMatchChecksum } from '../core/checksum';
import { BroadcastChannelTransport } from '../core/localTransport';
import { createInitialMatchState, type MatchEventValue, type MatchState } from '../core/matchState';
...
export class NetworkBoardScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => this.closeNetwork());
    this.events.once(Phaser.Scenes.Events.DESTROY, () => this.closeNetwork());
    this.refreshHud();
  }
  ...
  private setupNetwork(): void {
    const config = browserSession.current;
    const transport = new BroadcastChannelTransport<TwoTabMessage>(
    ...
    if (config.mode === 'host') {
      ...
      this.hostSession = new TwoTabHostSession(config.roomCode, authority, transport);
      this.unsubscribeSession = this.hostSession.subscribe((event) => this.handleSessionEvent(event));
      this.hostSession.start();
      this.writeLog(`📡 HOST ${config.roomCode}: mở tab khác → JOIN cùng room code.`);
      return;
    }
    ...
    this.unsubscribeSession = this.clientSession.subscribe((event) => this.handleSessionEvent(event));
    this.clientSession.start();
    this.writeLog(`🛰️ CLIENT P${config.seatId + 1}: đang chờ host ${config.roomCode}...`);
  }
  ...
  private handleSessionEvent(event: TwoTabSessionEvent): void {
  ...
  private applyNetworkState(
    ...
    if (source !== 'snapshot') {
      this.writeLog(`🔄 ${source.toUpperCase()} → cmd #${commandSeq} • ${checksum}`);
    }
    this.refreshHud();

    if (this.phase.is('BRANCH_CHOICE') && this.canControlCurrentPlayer()) {
  ...
  private networkStatusLabel(commandSeq?: number, checksum?: string): string {
    const config = browserSession.current;
    const role = config.mode === 'host' ? 'HOST' : `CLIENT P${config.seatId + 1}`;
    ...
    return `📡 ${role} • ROOM ${config.roomCode}${joined}${suffix}`;
  }
  ...
  private submitIntent(type: ClientIntentType, data: Record<string, MatchEventValue> = {}): void {
    ...
    try {
      if (this.hostSession) {
        this.hostSession.submitLocalIntent(type, current.id, data);
      } else if (this.clientSession) {
        this.clientSession.submitIntent(type, data);
      }
    } catch (error) {
  ...
  private async promptNetworkBranch(): Promise<void> {
    if (this.branchPromptOpen || !this.phase.is('BRANCH_CHOICE') || !this.canControlCurrentPlayer()) return;
  ...
  private drawHeader(): void {
    ...
    this.add.text(178, 45, 'CITY • MVP 0.1.14', {
      fontFamily: 'Arial, sans-serif',
      fontSize: '24px',
      fontStyle: 'bold',
      color: '#202020',
    });
    this.add.text(178, 75, 'TWO-TAB LOCAL SESSION • CLIENT INTENT → HOST AUTHORITY', {
      fontFamily: 'Arial, sans-serif',
      fontSize: '14px',
      fontStyle: 'bold',
      color: '#6d655b',
  ...
  private drawBoard(): void {
    ...
    this.add
      .text(640, 350, 'LOCAL MULTIPLAYER\nHOST AUTHORITY', {
        fontFamily: 'Arial, sans-serif',
        fontSize: '28px',
        fontStyle: 'bold',
        color: '#c8b99f',
  ...
  private createHud(): void {
    ...
    this.add.text(640, 505, 'SPACE roll • C cards • host điều khiển ghế chưa có client', {
  ...
  private refreshHud(): void {
    ...
    this.phaseText.setText(`${phaseLabel} • rng ${this.match.rng.calls} • ${checksum}`);
    ...
    this.scoreText.setText(
      this.players.map((player, index) => {
        ...
        const hostOwned = config.mode === 'host' && this.hostSession?.controlsActor(player.id) ? ' H' : '';
    ...
    this.rollButtonText.setText(canRoll ? 'ĐỔ XÚC XẮC' : canControl ? phaseLabel : `CHỜ ${this.shortName(current.name)}`);
