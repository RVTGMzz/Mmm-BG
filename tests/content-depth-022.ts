...
assert(cards.length >= 10, '0.1.22 should expose a visibly larger Card pool');
assert(news.length >= 8, '0.1.22 should expose a visibly larger News pool');
...
assert.equal(groupNewsWeight, 100);
...
assert.equal(drawWeightedNews(news, () => 0.95)?.effect.type, 'money_delta_all');
...
console.log('[content-depth-022] PASS expanded Card/News pools preserve probability bands and city tiles have identities');
