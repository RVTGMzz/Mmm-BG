...
import { applyCardEffect, getValidTargets, type CardDefinition } from '../src/core/cards';
import { advanceMatchTurn, createInitialMatchState, type MatchCommand } from '../src/core/matchState';
...
import { ECONOMY_060, MVP_TARGET_LAPS_060, PACING_060, isPlayerFinished060, miniGameEligiblePlayers060 } from '../src/core/pacingEconomy060';
...
const defaultState = createInitialMatchState({ boardId: BOARD.id, startNodeId: BOARD.startNodeId, playerNames: ['P1', 'P2', 'P3', 'P4'], seed: 6001 });
assert.deepEqual(defaultState.players.map((player) => player.money), [200, 200, 200, 200]);
const mainMoney = BOARD.nodes.filter((node) => node.id >= 0 && node.id < 44 && node.type === 'money').map((node) => node.value ?? 0).sort((a, b) => a - b);
...
for (const slot of MINI_GAME_SLOTS_059) {
  assert.equal(miniGameRewardTable059(slot.contentId, 'majority_minority').reduce((sum, amount) => sum + amount, 0), ECONOMY_060.miniGameMajorityTotal);
  assert.equal(miniGameRewardTable059(slot.contentId, 'rps').slice(0, 2).reduce((sum, amount) => sum + amount, 0), ECONOMY_060.miniGameRpsTotal);
}
...
assert.deepEqual(selfNews.map((news) => news.effect.type === 'money_delta_self' ? news.effect.amount : 0).sort((a, b) => a - b), [-30, -30, -30, 25, 25, 25]);
const groupNews = NEWS.filter((news) => news.effect.type === 'money_delta_all').map((news) => news.effect.type === 'money_delta_all' ? news.effect.amount : 0).sort((a, b) => a - b);
...
assert.deepEqual(getValidTargets(players, 1).map((player) => player.id), [2, 3]);
assert.deepEqual(miniGameEligiblePlayers060(players).map((player) => player.id), [1, 3]);
const groupGain: NewsDefinition = { id: 'TEST_060_GROUP', title: 'Test', rarity: 'N', dropWeight: 1, impact: '⭐', description: '', targetMode: 'all_players', effect: { type: 'money_delta_all', amount: 10 } };
applyNewsEffect(groupGain, players[1]!, players);
assert.equal(players[0]!.money, 320); assert.equal(players[1]!.money, 210); assert.equal(players[2]!.money, 190); assert.equal(players[3]!.money, 230);
const groupLoss = CARDS.find((card) => card.effect.type === 'percent_loss_all_others')!;
applyCardEffect(groupLoss, players[1]!, players);
assert.equal(players[0]!.money, 320);
const jailCard = CARDS.find((card) => card.id === 'ACT_017')!;
assert.throws(() => applyCardEffect(jailCard, players[1]!, players, players[0]), /final B\$ is already locked/i);
const orderState = createInitialMatchState({ boardId: 'turn-skip-060', startNodeId: 0, playerNames: ['P1', 'P2', 'P3', 'P4'], seed: 6002, playOrder: [0, 1, 2, 3] });
orderState.players[0]!.lapsCompleted = 1; orderState.turn.currentPlayerIndex = 3;
assert.equal(advanceMatchTurn(orderState), 1);
const finishBoard: BoardDefinition = { id: 'finish-lock-060', name: '0.1.60 finish lock fixture', startNodeId: 0, nodes: [{ id: 0, x: 0, y: 0, type: 'ready', contentId: 'READY' }, { id: 1, x: 10, y: 0, type: 'normal', contentId: 'M02' }], edges: [{ from: 0, to: 1, route: 'main' }, { from: 1, to: 0, route: 'main' }] };
let finishSeed = 1;
for (; finishSeed < 10_000; finishSeed += 1) { const probe = createInitialMatchState({ boardId: finishBoard.id, startNodeId: 0, playerNames: ['P1', 'P2'], seed: finishSeed }); const random = createRandomSource(probe.rng); if (rollD6(random) >= 3) break; }
assert(finishSeed < 10_000);
const finishSource = createInitialMatchState({ boardId: finishBoard.id, startNodeId: 0, playerNames: ['P1', 'P2'], seed: finishSeed });
const finishCommand: MatchCommand = { seq: 1, type: 'roll', turnNumber: 1, playerIndex: 0, actorId: 0, data: {} };
finishSource.commandLog = [finishCommand]; finishSource.nextCommandSeq = 2;
...
assert(rolled >= 3); assert.equal(finishReplay.state.players[0]!.nodeId, 0); assert.equal(finishReplay.state.players[0]!.lapsCompleted, 1);
assert.equal(finishReplay.state.eventLog.filter((event) => event.type === 'move_step').length, 2);
assert.equal(finishReplay.state.eventLog.find((event) => event.type === 'ready_pass')?.data.finishLocked, true);
assert.equal(finishReplay.state.turn.currentPlayerIndex, 1);
assert(PACING_060.globalAutoMaxMs < 10_000); assert(PACING_060.passiveAutoMaxMs < 6_000);
