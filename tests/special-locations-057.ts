...
function jailBoard(): BoardDefinition {
  return {
    id: 'special-jail-057',
    name: 'Jail 0.1.57 authority fixture',
    startNodeId: 0,
    nodes: [
      ...
      { id: 101, x: 30, y: 0, type: 'normal', contentId: 'JAIL_EXIT_1' },
      { id: 102, x: 40, y: 0, type: 'normal', contentId: 'JAIL_EXIT_2' },
      { id: 103, x: 50, y: 0, type: 'normal', contentId: 'JAIL_EXIT_3' },
...
function hospitalBoard(): BoardDefinition {
  return {
    ...
    name: 'Hospital 0.1.57 authority fixture',
    startNodeId: 0,
    nodes: [
      { id: 0, x: 0, y: 0, type: 'normal' },
      { id: 1, x: 10, y: 0, type: 'normal', contentId: 'SPECIAL_HOSPITAL_GATE' },
      { id: 110, x: 20, y: 0, type: 'normal', contentId: 'SPECIAL_HOSPITAL_HOLD' },
      { id: 111, x: 30, y: 0, type: 'normal', contentId: 'HOSPITAL_EXIT_1' },
      { id: 112, x: 40, y: 0, type: 'normal', contentId: 'HOSPITAL_EXIT_2' },
      { id: 113, x: 50, y: 0, type: 'normal', contentId: 'HOSPITAL_EXIT_3' },
...
assert.deepEqual(specialReleasePath057('jail'), [101, 102, 103, 12]);
assert.deepEqual(specialReleasePath057('hospital'), [111, 112, 113, 34]);
...
assert.equal(jailSuccess.state.players[0]!.nodeId, 12, 'release path must rejoin at M13/node 12');
...
// Seed 11 second face 2 succeeds for Hospital.
...
assert.equal(hospitalSuccess.state.players[0]!.nodeId, 34, 'Hospital release must rejoin at M35/node 34');
...
console.log('[special-locations-057] PASS Jail/Hospital release + fresh D6 + Lottery x20 + Mini Game eligibility + checksum');
