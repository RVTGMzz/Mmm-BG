...
assert.match(board, /setMaxLines\(0\)/, 'Job result detail must allow multiple readable lines');
...
assert.match(board, /setPosition\(0, -28\)/);
assert.match(board, /setPosition\(0, 22\)/);
assert.match(board, /jobDetailCopy/);
assert.match(board, /setFixedSize\(500, 92\)/);
assert.match(board, /setWordWrapWidth\(500, true\)/);
...
assert.match(setup, /Avatar người chơi • Có thể bỏ qua/);
