...
assert.match(setup, /face-menu-\$\{playerId\}/);
assert.match(setup, /CHỌN ẢNH/);
assert.match(setup, /CHỌN CÁCH TẠO AVATAR/);
assert.match(setup, />1 ẢNH<\/span>/);
assert.match(setup, /handleFaceBatchSelection/);
assert.match(setup, /files\.slice\(0, 3\)/);
const interactionCss = readFileSync('src/uiInteraction07046.css', 'utf8');
assert.match(interactionCss, /face-picker-launch/);
assert.match(interactionCss, /face-choice-grid/);
assert.match(setup, />CHỤP 3<\/span>/);
assert.match(setup, />CHỤP 1<\/span>/);
