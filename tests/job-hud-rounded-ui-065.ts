...
assert.match(mainSource,/CareerMinigameBoardScene069 as ActiveBoardScene/); assert.match(scene069,/extends CareerMinigameBoardScene0682/); assert(!scene069.includes('Math.random')); assert(!scene069.includes('submitIntent('));
console.log('[job-hud-rounded-ui-065] PASS historical salary + rounded proxy UI retained beneath 0.1.69 owned HUD');
