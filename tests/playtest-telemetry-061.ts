...
const match = createInitialMatchState({ boardId: 'telemetry-061-fixture', startNodeId: 0, playerNames: ['P1', 'P2', 'P3', 'P4'], seed: 6101 });
match.players[0]!.money = 260; match.players[1]!.money = 220; match.players[2]!.money = 180; match.players[3]!.money = 240;
...
match.commandLog = commands; match.nextCommandSeq = 7;
...
[25, 15, 10, 0].forEach((amount, index) => appendMatchEvent(match, 'minigame_reward', { amount, rank: index + 1, affectedPlayerIds: String(index) }, index));
...
for (const actorId of [2, 0, 3, 1]) appendMatchEvent(match, 'ready_pass', { finishLocked: true, lapsCompleted: 1, affectedPlayerIds: String(actorId) }, actorId);

const checksumBefore = computeMatchChecksum(match);
const report = buildPlaytestMatchReport061(match);
assert.equal(computeMatchChecksum(match), checksumBefore, 'building telemetry must not mutate authoritative state');
...
assert.equal(report.seed, 6101); assert.equal(report.checksum, checksumBefore); assert.equal(report.playerCount, 4); assert.equal(report.startingMoney, 200);
assert.equal(report.turnsObserved, 6); assert.equal(report.commandCount, 6); assert.equal(report.rngCalls, 17); assert.equal(report.movementRolls, 1);
assert.equal(report.releaseRolls, 1); assert.equal(report.lotteryRolls, 1); assert.equal(report.cardsPlayed, 1); assert.equal(report.newsTriggered, 1);
assert.equal(report.miniGamesTriggered, 1); assert.equal(report.miniGamesSkipped, 0); assert.equal(report.miniGameRewardTotal, 50);
assert.equal(report.jobOffers, 1); assert.equal(report.jobsSelected, 1); assert.equal(report.jobProgressChecks, 1); assert.equal(report.jailReleaseAttempts, 1); assert.equal(report.hospitalReleaseAttempts, 0);
assert.equal(report.lotteryCount, 1); assert.equal(report.lotteryRewardTotal, 60); assert.equal(report.finalMoneyTotal, 900); assert.equal(report.finalMoneyAverage, 225); assert.equal(report.finalMoneySpread, 80);
assert.deepEqual(report.finishOrderPlayerIds, [2, 0, 3, 1]); assert.deepEqual(report.players.map((player) => player.moneyDelta), [60, 20, -20, 40]); assert.deepEqual(report.players.map((player) => player.finishOrder), [2, 4, 1, 3]);
const text = formatPlaytestMatchReport061(report);
assert(text.includes('MeMeMe PLAYTEST REPORT 0.1.61')); assert(text.includes('seed=6101')); assert(text.includes(`checksum=${checksumBefore}`)); assert(text.includes('mini=+50B$')); assert(text.includes('spread=80B$')); assert(text.includes('finish: #1 P3 > #2 P1 > #3 P4 > #4 P2')); assert(text.includes('P1 P1: 260 B$ (+60)'));
...
console.log('[playtest-telemetry-061] PASS deterministic local-only report retained beneath 0.1.70.1 presentation wrapper');
