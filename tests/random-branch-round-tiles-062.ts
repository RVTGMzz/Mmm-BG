...
for (let seed = 620000; seed < 620256 && observed.size < 2; seed += 1) {
...
assert(observed.has('odd'), 'Could not observe an odd LEFT auto-branch in deterministic seed sweep.');
assert(observed.has('even'), 'Could not observe an even RIGHT auto-branch in deterministic seed sweep.');

const firstJunctionEdges = getOutgoingEdges(BOARD, junctions[0]!.id);
const oddTarget = observed.get('odd')!.to;
const evenTarget = observed.get('even')!.to;
assert(firstJunctionEdges.find((edge) => edge.to === oddTarget)?.label?.includes('TRÁI'), 'Observed odd HOST branch did not go LEFT.');
assert(firstJunctionEdges.find((edge) => edge.to === evenTarget)?.label?.includes('PHẢI'), 'Observed even HOST branch did not go RIGHT.');
